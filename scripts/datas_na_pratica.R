...
plot_ly(predLL) %>% 
  ...
  layout(title = 'Predição de mortes por COVID 19 em Pernambuco', 
...
covidPE <- covidPE %>% mutate(new_deaths_sMM7 = round(rollmean(x = deaths, 7, align = "right", fill = NA), 2)) # média móvel de 7 dias
